...
class ScheduleManager:
    ...
    def _load_data(self):
        """
        Loads persistent data from the JSON file into memory.

        - Converts JSON objects into actual class instances (e.g. StudentUser, Course, etc.).
        - Ensures fallback to a valid default structure if the file is missing or corrupted.
        - Avoids app crash on startup due to invalid file format or missing keys.
        """
        try:
            with open(self.data_path, 'r') as f:
                data = json.load(f)

            # Deserialize data into proper Python objects with type safety
            self.students = [StudentUser(**s) for s in data.get("students", []) if isinstance(s, dict)]
            self.teachers = [TeacherUser(**t) for t in data.get("teachers", []) if isinstance(t, dict)]
            self.courses = [Course(**c) for c in data.get("courses", []) if isinstance(c, dict)]
            self.attendance_log = data.get("attendance", [])
            self.instruments = data.get("instruments", [])
            self.admin = [AdminUser(**a) for a in data.get("admin", []) if isinstance(a, dict)]
            self.staff = [StaffUser(**sa) for sa in data.get("staff", []) if isinstance(sa, dict)]

            # Load next ID counters (or fallback to default if missing)
            self.next_student_id = int(data.get("next_student_id", 4000))
            self.next_teacher_id = int(data.get("next_teacher_id", 3000))
            self.next_admin_id = int(data.get("next_admin_id", 1000))
            self.next_staff_id = int(data.get("next_staff_id", 2000))

            print("Data loaded successfully from file.")

        except (FileNotFoundError, json.JSONDecodeError):
            # Fallback: create new data file if missing or corrupted
            print("Data file not found or invalid. Initializing empty data structure.")
            self._save_data()
